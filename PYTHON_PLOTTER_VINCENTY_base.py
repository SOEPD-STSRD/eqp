...
def DELAZ_orig(slat, slon, eplat, eplon):
...
for ITER in range(ITERAN):
	...
	if (SUMWT == 0):
		...
		if (ITLAST != 0):
			...
			# print (ITER, OLAT, OLON, DEPP, IAH, IAM, ASEC, RMS, ABADA, ABADE, ABADI, ABADO)
			break
	...
	ABADA = format(round(math.sqrt(BDIAG[1]) * RMS / 111.2, 4), '.4f')
	ABADE = format(round(math.sqrt(BDIAG[2]) * RMS / 111.2, 4), '.4f')
	ABADI = format(round(math.sqrt(BDIAG[3]) * RMS, 2), '.2f')
	ABADO = format(round(math.sqrt(BDIAG[0]) * RMS, 2), '.2f')
