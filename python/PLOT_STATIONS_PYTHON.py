...
def within200(EPICENTER):
	D = 0
	with open("C:/EQP11-20/Programs/PHILSTA.DAT") as f:
		for line in f:
			lat = float(line[8:14])
			lon = float(line[17:24])
			delta, azim = DELAZ(lat, lon, float(EPICENTER[0]), float(EPICENTER[1]))
			if (float(delta)*111.2 <= 200):
				D += 1
	return D
...
start = time.time()
STATMAIN = 'C:/EQP11-20/Programs/PHILSTA-main.DAT'
...
stat1 = 'C:/EQP11-20/Programs/PLOT.DAT'
statname1, P, S = get_station_locations(stat1)

plot_out = 'C:/EQP11-20/Programs/PLOT.OUT'
...
eplat, eplon = epicenter_location('C:/EQP11-20/Programs/PLOT.OUT')
STA, TOA, AZIM, DELTA, RESP, RESS, OBSP, OBSS, CALCP, CALCS = get_solution('C:/EQP11-20/Programs/PLOT.OUT')
TTMTAB, HEADER, NUMDEP, DEP1ST, DDELT, NUMDTA, DEP = read_table('C:/EQP11-20/Programs/JBTAB.PS')
DEPTH = get_depth('C:/EQP11-20/Programs/PLOT.OUT')
...
print ("Gap Angle: " + str(gap) + " between " + stations[0][:3] + " and " + stations[1][:3])
...
fig.text(text=stations[0][:3] + " and " + stations[1][:3], x=16.5, y=16, font="10p,Helvetica", justify='ML')
